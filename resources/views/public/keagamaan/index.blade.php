...
@section('title', kua_setting('instansi', 'Surat Digital KUA').' — '.kua_navbar_page_label('keagamaan', $page->title ?? 'Layanan Keagamaan'))

@section('content')
    <section class="mx-auto max-w-4xl px-6 pb-16 pt-12">
        <h1 class="text-center text-2xl font-bold">{{ $page->title ?? 'Layanan Keagamaan' }}</h1>
        <p class="mx-auto mt-2 max-w-2xl text-center text-sm text-[#1b1b1870]">
            {{ $page->description ?? 'Pilih topik di bawah untuk melihat persyaratan, alur, dan prosedur layanan keagamaan di KUA.' }}
        </p>

        <div class="mt-8 space-y-4">
            @forelse ($religiousServices as $service)
            ...
            @empty
                <div class="rounded-lg border border-teal-100 bg-white p-8 text-center text-sm text-[#1b1b1870]">
                    Belum ada layanan keagamaan yang tersedia.
                </div>
            @endforelse
        </div>
    </section>

...
@php($layananDropdown = $layananItem && $layananItem->has_submenu && $headerServices->isNotEmpty())
@php($tentangDropdown = $tentangItem && $tentangItem->has_submenu && $tentangItems->isNotEmpty())
    ...
    <div class="mx-auto flex max-w-5xl items-center justify-between px-6 py-3">
        ...
        <nav x-data="{ layanan: false, tentang: false }" @click.outside="layanan = false; tentang = false" class="hidden items-center gap-2 text-sm font-medium sm:flex">
            @foreach ($navbarItems as $item)
                @if ($item->key === 'layanan' && $layananDropdown)
                ...
                @elseif ($item->key === 'tentang' && $tentangDropdown)

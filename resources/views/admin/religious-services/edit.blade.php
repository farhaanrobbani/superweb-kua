<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Topik Keagamaan</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('religious-services.update', $religiousService) }}" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <x-text-input id="name" name="name" required maxlength="150"
                                          value="{{ old('name', $religiousService->name) }}" class="mt-1 block w-full" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="2" maxlength="2000"
                                      class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm text-sm">{{ old('description', $religiousService->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="icon" id="icon"
                                ...
                                @foreach (\App\Http\Controllers\Admin\ReligiousServiceController::ICONS as $key => $label)
                                    <option value="{{ $key }}" @selected(old('icon', $religiousService->icon ?: 'heart') === $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                <x-text-input id="persyaratan_label" name="persyaratan_label" maxlength="50"
                                              value="{{ old('persyaratan_label', $religiousService->persyaratan_label) }}" class="mt-1 block w-full"
                            ...
                            <div>
                                ...
                                <x-text-input id="alur_label" name="alur_label" maxlength="50"
                                              value="{{ old('alur_label', $religiousService->alur_label) }}" class="mt-1 block w-full"
                                              placeholder="Alur" />
                            ...
                            <div>
                                ...
                                <x-text-input id="sop_label" name="sop_label" maxlength="50"
                                              value="{{ old('sop_label', $religiousService->sop_label) }}" class="mt-1 block w-full"
                                              placeholder="SOP" />
                        ...
                        <div>
                            ...
                            <textarea id="persyaratan" name="persyaratan" data-editor rows="8"
                                      ...
                                      class="block w-full">{{ old('persyaratan', $religiousService->persyaratan) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="alur" name="alur" data-editor rows="8"
                                      data-upload-url="{{ route('announcements.gambar') }}"
                                      class="block w-full">{{ old('alur', $religiousService->alur) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="sop" name="sop" data-editor rows="8"
                                      data-upload-url="{{ route('announcements.gambar') }}"
                                      class="block w-full">{{ old('sop', $religiousService->sop) }}</textarea>
                ...
                <div class="bg-white rounded-lg shadow-sm dark:bg-gray-800">
                    <div class="px-4 py-4 space-y-4">
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <select name="active" id="active"
                                        class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm text-sm">
                                    <option value="1" @selected(old('active', $religiousService->active ? '1' : '0') === '1')>Aktif (ditampilkan)</option>
                                    <option value="0" @selected(old('active', $religiousService->active ? '1' : '0') === '0')>Nonaktif (draf)</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <x-text-input id="sort_order" name="sort_order" type="number" min="0" max="9999"
                                              value="{{ old('sort_order', $religiousService->sort_order) }}" class="mt-1 block w-full" />
                        ...
                        <div class="pt-2 border-t border-gray-100 dark:border-gray-700 flex items-center gap-3">
                            ...
                            <a href="{{ route('pages.index', ['tab' => 'keagamaan']) }}" class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-500 hover:underline">Batal</a>
                        </div>
                    </div>
                </div>
            </form>

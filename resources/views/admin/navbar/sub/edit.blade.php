<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Sub Menu {{ $parent?->label }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm dark:bg-gray-800 sm:rounded-lg p-6">
                <form method="POST" action="{{ route('navbar.sub.update', $subItem) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="url" name="url" class="mt-1 block w-full" maxlength="255"
                                      value="{{ old('url', $subItem->url) }}" placeholder="/halaman" />
                        <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">Tujuan tautan sub menu, misalnya /permohonan atau tautan eksternal.</p>
                    ...
                    <div class="mt-4">
                        ...
                        <select id="icon" name="icon"
                            ...
                            @foreach (\App\Http\Controllers\Admin\NavbarController::icons() as $key => $label)
                                <option value="{{ $key }}" @selected(old('icon', $subItem->icon) === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="sort_order" name="sort_order" type="number" min="0" max="9999"
                                      class="mt-1 block w-full" value="{{ old('sort_order', $subItem->sort_order) }}" />
                    ...
                    <div class="mt-4">
                        <label class="inline-flex items-center">
                            ...
                            <span class="ms-2 text-sm text-gray-600 dark:text-gray-300 dark:text-gray-500">Aktif</span>
                        </label>
                    </div>

                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($templates as $template)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $template->letterType->name }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded-full {{ $template->active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                        {{ $template->active ? 'Aktif' : 'Nonaktif' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm space-x-2">
                                    <a href="{{ route('letter-templates.edit', $template) }}" class="text-blue-600 hover:underline">Edit</a>

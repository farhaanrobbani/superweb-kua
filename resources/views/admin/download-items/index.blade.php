<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Download Center</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex justify-end">
                <a href="{{ route('download-items.create') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700/40">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase">Sumber</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-800">
                        @forelse ($downloadItems as $item)
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                    <span class="font-medium">{{ $item->title }}</span>
                                    @if ($item->description)
                                        <p class="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">{{ str($item->description)->limit(80) }}</p>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{{ $item->category ?: '—' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">
                                    @if ($item->file)
                                        <span class="text-xs">{{ $item->fileName() }} ({{ $item->fileSize() ?? '—' }})</span>
                                    @else
                                        <span class="text-xs text-blue-600 dark:text-blue-400">Link eksternal</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm space-x-2">
                                    <a href="{{ route('download-items.edit', $item) }}" class="text-blue-600 dark:text-blue-400 hover:underline">Edit</a>
                                    <form action="{{ route('download-items.destroy', $item) }}" method="POST" class="inline"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">Belum ada berkas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-4">{{ $downloadItems->links() }}</div>
        </div>
    </div>
</x-app-layout>

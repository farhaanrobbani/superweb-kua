<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Topik Wakaf</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('wakaf-services.update', $wakafService) }}" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <x-text-input id="name" name="name" required maxlength="150"
                                          value="{{ old('name', $wakafService->name) }}" class="mt-1 block w-full" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="2" maxlength="2000"
                                      class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm text-sm">{{ old('description', $wakafService->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="icon" id="icon"
                                ...
                                @foreach (\App\Http\Controllers\Admin\WakafServiceController::ICONS as $key => $label)
                                    <option value="{{ $key }}" @selected(old('icon', $wakafService->icon ?: 'document') === $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid gap-4 sm:grid-cols-3">
                            <div>
                                ...
                                <x-text-input id="persyaratan_label" name="persyaratan_label" maxlength="50"
                                              value="{{ old('persyaratan_label', $wakafService->persyaratan_label) }}" class="mt-1 block w-full"
                            ...
                            <div>
                                ...
                                <x-text-input id="alur_label" name="alur_label" maxlength="50"
                                              value="{{ old('alur_label', $wakafService->alur_label) }}" class="mt-1 block w-full"
                                              placeholder="Alur" />
                            ...
                            <div>
                                ...
                                <x-text-input id="sop_label" name="sop_label" maxlength="50"
                                              value="{{ old('sop_label', $wakafService->sop_label) }}" class="mt-1 block w-full"
                                              placeholder="SOP" />
                        ...
                        <div>
                            ...
                            <textarea id="persyaratan" name="persyaratan" data-editor rows="8"
                                      ...
                                      class="block w-full">{{ old('persyaratan', $wakafService->persyaratan) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="alur" name="alur" data-editor rows="8"
                                      data-upload-url="{{ route('announcements.gambar') }}"
                                      class="block w-full">{{ old('alur', $wakafService->alur) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="sop" name="sop" data-editor rows="8"
                                      data-upload-url="{{ route('announcements.gambar') }}"
                                      class="block w-full">{{ old('sop', $wakafService->sop) }}</textarea>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <select name="active" id="active"
                                        class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm text-sm">
                                    <option value="1" @selected(old('active', $wakafService->active ? '1' : '0') === '1')>Aktif (ditampilkan)</option>
                                    <option value="0" @selected(old('active', $wakafService->active ? '1' : '0') === '0')>Nonaktif (draf)</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <x-text-input id="sort_order" name="sort_order" type="number" min="0" max="9999"
                                              value="{{ old('sort_order', $wakafService->sort_order) }}" class="mt-1 block w-full" />
